    ...
    virtual void _appendField(StrSlice slice, size_t field_num, ColumnSet* columns) {
        ...
        switch (column->getType()) {
            case ColumnType::STRING:
                ...
                dynamic_cast<StringCol*>(column)->append(slice.toCString());
                break;
            case ColumnType::INTEGER:
                dynamic_cast<IntegerCol*>(column)->append(slice.toInt());
                break;
            case ColumnType::FLOAT:
                dynamic_cast<FloatCol*>(column)->append(slice.toFloat());
                break;
            case ColumnType::BOOL:
                dynamic_cast<BoolCol*>(column)->append(slice.toInt() == 1);
                break;
            default:
                assert(false);
        }
